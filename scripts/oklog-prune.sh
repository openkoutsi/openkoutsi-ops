...
# Safe to run by hand.
#
# Vector writes daily files at ${DATA_MOUNT}/service_logs/<container>/<date>.log.
# Anything older than LOG_RETENTION_DAYS is deleted; empty dirs are tidied up.

set -euo pipefail

# Non-secret config (DATA_MOUNT, LOG_RETENTION_DAYS).
set -a
# shellcheck disable=SC1091
. /opt/openkoutsi/stack.env
set +a

LOG_DIR="${DATA_MOUNT}/service_logs"
